...
with extensive experience building systems for managing Linux, cloud and
bare-metal infrastructure at scale. Strong background in configuration
management, provisioning, secrets, observability, CI/CD and cross-team
...
* Developed tools to provision, configure, monitor and support globally
  ...
  data centers. Mentored junior and non-traditional developers (sysadmin,
...
* Architected and maintained a *Python* monorepo hosting 50 projects from a
  dozen contributors. Standardized development patterns (dependency management,
  testing, packaging) through tooling and applied those tools in a
  comprehensive *GitLab CI* pipeline to achieve efficient builds based on
...
* Led integration with firmwide *HashiCorp Vault* for global secrets management
  ...
  *Telegraf* and *Grafana*), managed global deployment and supported application
...
* Led design (through a collaborative design doc review process) and
  implementation of *Ansible* runner tool in *Python* which served as the
  primary entrypoint for 1000s of executions of `ansible-playbook` weekly
  providing increased observability (*InfluxDB*, *Grafana*), simplifying
  understanding of failures across dozens of environments and decreasing time
  to resolution.
* Built a *Git* workflow for batch job configuration on a proprietary scheduler
  replacing a *CVS* workflow which improved change visibility, developer
...
* Led design and implementation of patterns and best practices for managing
  ...
  *Mesos*/*Aurora*, *HDFS* and *Airflow*) deployed across 6 environments and 2
  public clouds (*AWS*, *Azure*).
* Led strategic planning and implementation of EOL OS upgrades across all
  ...
  efforts.
* Defined *Terraform* module architecture, moving from monolithic, repetitive
  environment configs to composable, reusable modules simplifying how
  infrastructure is developed and tested for developers.
* Built *Clojure* tooling for managing large *Terraform* configs with state
...
* Implemented infrastructure tooling for provisioning (*AWS CloudFormation*,
  ...
  *Marathon*, *Chronos* and *Docker*), monitoring and alerting (*Sensu*,
  *Graphite* and *Grafana*) and security and compliance.
* Led implementation of an internet-facing, authenticated reverse proxy
  (*nginx*, *OpenResty* and *OIDC*) for operational dashboards eliminating VPN
...
* Implemented and maintained platform tooling for infrastructure (hosted on
  ...
  *Kibana*), configuration management and deployment automation (*Ansible* and
  ...
  *Redis* and *Couchbase*) and monitoring and alerting (*InfluxDB*, *Grafana*,
  *Graphite* and *Nagios*).
* Designed framework and led network integrations (*Amazon VPC* and *strongSwan
  IPSec*) with large German and foreign retailers to provide access to internal
  systems facilitating new client onboarding.
...
* Led migration from *EC2 Classic* to *EC2 VPC* moving both stateful and
  stateless clusters across multiple environments without downtime, optimizing
...
* Full-stack developer on large, highly trafficked, multi-market, editorial
  *Ruby on Rails* website based on *MySQL*, *Memcache*, *Solr*, *jQuery*,
  *PrototypeJS* and *Capistrano*.
* Major contributor to *ETL* architecture based on *CouchDB* serving as
  foundation for new product serving markets without editorial staff for
  content generation.
